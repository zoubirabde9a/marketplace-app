...
2026-05-12 · vps-eu · web + api rebuild — edge HTML cache via Next middleware (s-maxage=300, swr=1800) on /, /about, /seller, /search, /product/{id}, /store/{id} for cookieless requests; logged-in SSR keeps private/no-store. Sitemap lastmod now uses ingestion time (search hit `updatedAt`) instead of Ouedkniss `sourcePostedAt` so freshly-surfaced URLs don't carry years-old freshness signals to Google. Checkout no longer auto-attaches FLAT_SHIPPING fee when client didn't pick a shipping option. Cart adds +/- adjustQtyAction. Verified live: api livez ok, / 200 emits public/s-maxage=300/swr=1800, sitemap.xml 200 with 50,104 urls. NOTE for operator: Vary: Cookie not surviving — framework Vary appears to be overwriting middleware's append; Cache-Control gating still safe (only emitted for cookieless reqs) but Cloudflare cache rule will want explicit cookie-absent match. Also note middleware-emitted s-maxage applies only to cookieless requests; logged-in users keep private/no-store.
