...
- **Frozen snapshot**: every call to `catalog.search`, `catalog.get_product`, `catalog.compare`, and `catalog.recommend` writes the exact tool output to a snapshot store keyed by an unguessable token (128 bits, base64url). The same applies to the equivalent REST reads `GET /v1/products` (kind `search`) and `GET /v1/products/{id}` (kind `product`), so a non-MCP client gets the same shareable link.
- **Link in tool response**: each response carries `snapshotUrl: {web_base}/s/{token}` plus `snapshotCreatedAt` and `snapshotExpiresAt` (epoch ms). The agent can paste the link into a chat with the human. The fields are only emitted when `MARKETPLACE_WEB_BASE_URL` is configured in the API/MCP environment; otherwise they are omitted (the snapshot is still written but no public link can be constructed).
