...
export function registerBuyerTools(reg: McpRegistry, deps: BuyerAdapter): void {
  ...
  reg.register({
    ...
    description: [
      ...
      "",
      "Getting `sellerId`: there is no `seller.list_mine` tool yet, so the agent MUST persist the",
      "`sellerId` returned by `seller.create_account` (e.g. ask the human operator to store it, or",
      "save it to a project file). If the operator says 'list my orders' and you don't have a",
      "`sellerId` on hand, ASK them for the shop's `sellerId` or `storeUrl` — do not guess one.",
    ].join("\n"),
