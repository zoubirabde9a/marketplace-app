...
**Catalog cap (as of 2026-05-17):** the loop holds the catalog at a fixed size. `--max-products 280000` is wired into the systemd service (`marketplace-scrape-loop.service`): each iteration seeds N fresh listings, then deletes the oldest scraper-created rows so the count stays at the cap. Prior value of 14,200 was raised to 280,000 once seller_id=NULL "unowned reference listings" became the seeding policy. Cascades clean up `catalog.media`, `catalog.product_variants`, and inventory rows. Image bytes are not stored locally — `catalog.media.url` holds the original Ouedkniss CDN URL — so pruning a product is a pure DB-row operation. Steady-state metric line shows `seeded=N pruned=N delta=0` once the catalog reaches the cap; until then `pruned=0` and the catalog grows. Timer fires every minute (`OnCalendar=*:0/1`) and rotates categories: telephones, informatique, electronique_electromenager, vetements_mode, immobilier, automobiles_vehicules, emploi_offres, services, meubles_maison, sport, sante_beaute, divers, pieces_detachees, materiaux_equipement, alimentaires, emploi_demandes, jeux_electroniques, loisirs_divertissements, voyages.
